...
from project_4.models import Task, db
from project_4.app import *
...
class MenuTests(BaseTests):
    ...
    def test_display_options_single_result(self):
        Task.create(employee="test_cruid", task="Writing Test Cases", startdate=datetime.datetime.now().date(),
    ...
    def test_display_options_multi_result(self):
        Task.create(employee="test_cruid", task="Writing Test Cases", startdate=datetime.datetime.now().date(),
                    duration=100, notes="I will handle this in no time")
        Task.create(employee="test_cruid2", task="Reading Test Cases", startdate=datetime.datetime.now().date(),
                    duration=300, notes="Easy Peasy")
        Task.create(employee="test_cruid3", task="Speaking", startdate=datetime.datetime.now().date(),
        ...
        with mock.patch('sys.stdout', new_callable=StringIO) as mock_stdout:
            with mock.patch('builtins.input', side_effect=['n', 'n', 'p','q']) as mock_input:
                ...
                self.assertIn("Press 'N'ext 'E'dit 'D'elete 'Q'uit ",mock_input.call_args_list[0][0][0])
                self.assertIn("Press 'N'ext 'P'revious 'E'dit 'D'elete 'Q'uit ", mock_input.call_args_list[1][0][0])
                self.assertIn("Press 'P'revious 'E'dit 'D'elete 'Q'uit ", mock_input.call_args_list[2][0][0])
...
class CRUDTests(BaseTests):
    def setUp(self):
        Task.create_table(safe=True)
        self.task1 = Task.create(employee="test_cruid", task="Writing Test Cases", startdate=datetime.datetime.now().date(),
                                 duration=100, notes="I will handle this in no time")
        ...
        self.task3 = Task.create(employee="test_other_employee", task="Reading", startdate=datetime.datetime.now().date(),
    ...
    def test_find_by_employee_entry(self, mock_stdout):
        ...
        with mock.patch('builtins.input', side_effect=user_input):
            with mock.patch('project_4.app.display_tasks', side_effect=[""]):
                find_by_employee()
                self.assertIn("test_cruid".capitalize() ,mock_stdout.getvalue())
                self.assertIn("has 2 entries", mock_stdout.getvalue()) # check the # of entries for that user
    ...
    def test_find_by_date(self):
        ...
        with mock.patch('builtins.input', return_value=user_input):
            with mock.patch('project_4.app.display_tasks') as mock_display_tasks:
                find_by_date()
    ...
    def test_edit_entry(self):
        ...
        task = Task.create(employee=employee, task=taskname, startdate=datetime.datetime.now().date(),
                    duration=300, notes="I will handle this in no time")
